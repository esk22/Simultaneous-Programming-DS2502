...
namespace DataManagerWindow
{
    public partial class MainForm : Form
    {
        ...
        private static bool goToNextForm;
        ...
        public MainForm()
        {
           ...
           PortList();
           // Auto detect the serial port connected
        ...
        private void InitializeComponent()
        {
            ...
            this.PortNumberDropList = new System.Windows.Forms.ComboBox();
            ...
            // Port_Selection
            // 
            this.Port_Selection.Controls.Add(this.PortNumberDropList);
            ...
            // PortNumberDropList
            // 
            this.PortNumberDropList.FormattingEnabled = true;
            this.PortNumberDropList.Location = new System.Drawing.Point(172, 64);
            this.PortNumberDropList.Name = "PortNumberDropList";
            this.PortNumberDropList.Size = new System.Drawing.Size(77, 25);
            this.PortNumberDropList.TabIndex = 4;
            this.PortNumberDropList.SelectedIndexChanged += new System.EventHandler(this.PortNumberDropList_SelectedIndexChanged);
            // 
            // label7
            // 
    ...
    "-Detect \r\nbelow.\r\n\r\n";
        ...
        // Intialize other components (includes customization and modification of windows)
        ...
        private void PortAutoDetect(SerialPort serialPort1)
        {
            ...
            {
                PortNumberDropList.Text = "  ";
                portDetected = false;
            }
        }

        // Append portname to the drop down list 
        // if it does not exist
        private void AppendPortToList(string portName)
        {
            bool PortExist = false;
            foreach (string port in PortNumberDropList.Items)
            {
                if(port == portName)
                {
                    PortExist = true;
            ...
            if (!PortExist)
                PortNumberDropList.Items.Add(portName);
        }
        ...
        private void Auto_botton_Click(object sender, EventArgs e)
        {
            ...
            if (portDetected)
            {
                ...
                PortNumberDropList.Text = portName;
            }
            else
            {
        ...
        //----------------------------------------------------------------------------------
        // Function name: private void PortList()
        ...
        private void PortList()
        {
            ...
            foreach(string port in ports)
                PortNumberDropList.Items.Add(port);
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (ports.Contains(portName))
            {
                for (int i = 0; i < 6; i++)
                    DataBuffer[i] = "";
                string selected = this.PortNumberDropList.GetItemText(this.PortNumberDropList.SelectedItem);
                ...
                else
                {
                    ...
                    {
                        // To next window -- 
                        // All functions are performed in the next window
                        GoToNextWindow(portName);
                    }
                    else
                    {
                        if (goToNextForm && isPortOpen && (selected == portName))
                        {
                            // To next window -- 
                            // All functions are performed in the next window
                            GoToNextWindow(portName);
            ...
            {
                PortNumberDropList.Text = "   ";
                PortNumberDropList.Items.Remove(portName);
                MessageBox.Show("Hardware device is disconnected !", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        //----------------------------------------------------------------------------------
        // Function name: private void GoToNextWindow(string portName)
        // Description: Go to the next window 
        //----------------------------------------------------------------------------------
        private void GoToNextWindow(string portName)
        {
            ...
            foreach (string element in DataBuffer)
            {
                ...
                {
                    // No ID Tag is connected to the
                    // hardware device
                    TagsExist = true;
                    break;
                }
                // else - at least one ID Tag is connected to the hardware device
            }
