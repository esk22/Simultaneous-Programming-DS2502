...
namespace DataManagerWindow
{
    public partial class DataViewWindow : Form
    {
        ...
        DataOutManager BytesOut;
        CRC crc;
        /// Constants
        private const int ProgressMax = 1950;
        private const int NUMBER_OF_TAGS = 6;
        private const int MEMORY_SIZE = 128;

        /// Main function
        public DataViewWindow(SerialPort serialPort, string portName, string[] StrDataBytes)
        {
            InitializeComponent();
            InitLocalComponent();
        ...
        private void InitLocalComponent()
        {
            ...
            BytesOut = new DataOutManager();
            crc = new CRC();

            UserChangedBarcode = new string[NUMBER_OF_TAGS];
            UserChangedDSR = new string[NUMBER_OF_TAGS];
            UserChangedCatalog = new string[NUMBER_OF_TAGS];
            ExistingDsrs = new string[NUMBER_OF_TAGS];
            HexList = new string[MEMORY_SIZE];
            BarcodeBuffer = new string[NUMBER_OF_TAGS];
            DsrBuffer = new string[NUMBER_OF_TAGS];
            CatalogBuffer = new string[NUMBER_OF_TAGS];
            RecentBarcode = new string[NUMBER_OF_TAGS];
            RecentDsr = new string[NUMBER_OF_TAGS];
            RecentCatalog = new string[NUMBER_OF_TAGS];
            CheckBoxBuffer = new bool[NUMBER_OF_TAGS];
            DataReplacedCount = new int[NUMBER_OF_TAGS];
            /// Define the size of the array that copies and holds
            /// the ID Tag data
            HexDataStr = new string[NUMBER_OF_TAGS];
            /// Init data record
            DataRecord = new string[NUMBER_OF_TAGS/2];
            BytesRemained = new int[NUMBER_OF_TAGS];
            for (int i = 0; i < NUMBER_OF_TAGS/2; i++)
            {
                DataRecord[i] = "a";
                BytesRemained[i] = MEMORY_SIZE;
                ExistingDsrs[i] = "";
            }
            for (int j = NUMBER_OF_TAGS / 2; j < NUMBER_OF_TAGS; j++ )
                BytesRemained[j] = MEMORY_SIZE;
        ...
        /// Description: 1) Init HexDataStr with input data string
        ///              2) Display data of first ID Tag in Record List and Hex Data text boxes
        ///              3) Initialize ID Tag checkboxs
        ///-----------------------------------------------------------------------------------------------------------
        private void InitDataBuffer(string[] StrDataBytes)
        {
            ...
            for (int i = 1; i <= StrDataBytes.Length; i++)
            {
                ...
                {
                    HexDataStr[i - 1] = BytesIn.SpaceDelimitor(BytesIn.RemoveExtraWords(StrDataBytes[i - 1]));
                    BytesIn.GetRecordList(HexDataStr[i - 1], ref DataRecord);
                    BufferExistingDsr(DataRecord, i - 1);
                    if (!DefaultText)
                    {
                        /// MessageBox.Show(StrDataBytes[i - 1]);
                        DisplayHexDataTextBox(HexDataStr[i - 1]);
                        /// BytesIn.GetRecordList(HexDataStr[i - 1], ref DataRecord);
                    ...
                    ComputeDataFields(HexDataStr[i - 1], i);
        ...
        /// Description: Record existing multiple DSR values in a buffer
        ///-----------------------------------------------------------------------------------------------------------
        ...
        ///              stored in an EPROM address of the ID Tag. The text box displays MEMORY_SIZE
        ...
        /// Description: Parse data string to appropriate data bytes and put in an array
        ///              - Each character in the string is a hex value
        ///              - Each data byte comes from a specific EPROM addresss
        ///                of the ID Tag (Example: 09 is a data byte) 
        ///                Example: the string 09310A contains 3 data bytes 09, 31, and 0A
        ///-----------------------------------------------------------------------------------------------------------
        ...
        /// Description: Compute the length and start position of a data in the EPROM
        ///-----------------------------------------------------------------------------------------------------------
        ...
        private void ComputeDataFields(string GetData, int tag)
        {
            ...
            string DataReplaced = "";
            string catalog = BytesIn.GetCatalogNumber(CatalogStr, ref DataReplaced);
        ...
        /// Description: Count the data bytes replaced most recently at the end of a Catalog number
        ...
        ///              characters long. Return true or false.
        ///-----------------------------------------------------------------------------------------------------------
        ...
        private bool CheckInputs(int i)
        {
            ...
            {
                if (!BytesOut.VerifyUserInputDsr(UserChangedDSR[i], ExistingDsrs[i]))
                {
        ...
        private void OkBotton_Click(object sender, EventArgs e)
        {
            ...
            InitCurrentbuffer();
            for (int i = 0; i < HexDataStr.Length; i++)
            {
        ...
        private void SendDataToPropeller()
        {
            ...
            for (int i = 0; i < CheckBoxBuffer.Length; i++)
            {
                if (CheckBoxBuffer[i])
                {
                    ...
                    string tag = BytesOut.GetTag(i);
                    /// Erase Barcode = x
                    /// Erase DSR = y
                    /// Erase Catalog number = z
                    string eraseFlag = "";
                    bool EraseBarcode = BytesOut.EraseData(BarcodeBuffer[i], UserChangedBarcode[i], "Barcode", ref eraseFlag);
                    bool EraseDsr = BytesOut.EraseData(DsrBuffer[i], UserChangedDSR[i], "DSR", ref eraseFlag);
                    bool EraseCatalog = BytesOut.EraseData(CatalogBuffer[i], UserChangedCatalog[i], "Catalog-number", ref eraseFlag);
                    if (EraseBarcode || EraseDsr || EraseCatalog)
                    {
                        string warning = "Do you want to erase " + BytesOut.DataFieldNames(eraseFlag) + " \r\n";
                        ...
                        {
                            string barcode = (EraseBarcode == true) ? "x" : BytesOut.GetDataToWrite(BarcodeBuffer[i], UserChangedBarcode[i]);
                            string dsr = (EraseDsr == true) ? "y" : BytesOut.GetDataToWrite(DsrBuffer[i], UserChangedDSR[i]);
                            /// Also, even if the DSR is erased, if there are multiple DSRs
                            /// then the next DSR value in the list will be the next available DSR value
                            if (EraseDsr)
                            {
                                RecentDsr[i] = BytesOut.GetChangedDsr(ExistingDsrs[i]);
                            }
                            string catalog;
                            
                            if (barcode.Length / 2 >= 7)
                                catalog = BytesOut.GetDataToWrite(CatalogBuffer[i], UserChangedCatalog[i], 1, DataReplacedCount[i]);
                            else if (dsr.Length / 2 >= 3 && dsr.Length / 2 <= 7)
                                catalog = BytesOut.GetDataToWrite(CatalogBuffer[i], UserChangedCatalog[i], 1, DataReplacedCount[i]);
                            else
                                catalog = (EraseCatalog == true) ? "z" : BytesOut.GetDataToWrite(CatalogBuffer[i], 
                    ...
                    {
                        string barcode = BytesOut.GetDataToWrite(BarcodeBuffer[i], UserChangedBarcode[i]);
                        string dsr = BytesOut.GetDataToWrite(DsrBuffer[i], UserChangedDSR[i]);
                        /// MessageBox.Show(DataReplacedCount[i].ToString() + " tag " + (i+1).ToString());
                        string catalog = BytesOut.GetDataToWrite(CatalogBuffer[i], UserChangedCatalog[i], (barcode + dsr).Length, DataReplacedCount[i]);
                        DataToWrite[i] = tag + ComputeDataToWrite(barcode, dsr, catalog, UserChangedBarcode[i], UserChangedDSR[i], UserChangedCatalog[i]);
            ...
            if (MemoryEnough)
            {
                if (BytesOut.DataFieldChanged(DataToWrite))
                {
        ...
        ///              of data bytes to be erased from and/or written into the EPROM.
        ///              2) Order the data bytes in sequence
        ...
        ///          Last byte (09) - Erase end address
        ///          s - Starting position of data bytes
        ...
        private void MarkDataTypes(int i, string tag)
        {
            ...
            {
                if (BYTES[a] == 'x')
                    bar_x = "x" + BytesOut.GetDataPosLen(HexDataStr[i], "Barcode");
                else if (BYTES[a] == 'y')
                    dsr_y = "y" + BytesOut.GetDataPosLen(HexDataStr[i], "DSR");
                else if (BYTES[a] == 'z')
                    cat_z = "z" + BytesOut.GetDataPosLen(HexDataStr[i], "Catalog");
                else
                {
                    data_bytes += BYTES[a];
                }
            ...
            if (data_bytes.Length > 1)
            {
                ...
                if (Convert.ToInt32(type, 16) == 8 || Convert.ToInt32(type, 16) == 9)
                    data_bytes = BytesOut.GetDataPosLen(HexDataStr[i], "Zap-Barcode") + data_bytes;
                else if (Convert.ToInt32(type, 16) >= 3 && Convert.ToInt32(type, 16) <= 7)
                    data_bytes = BytesOut.GetDataPosLen(HexDataStr[i], "Zap-DSR") + data_bytes;
                else if (Convert.ToInt32(type, 16) >= 10 && Convert.ToInt32(type, 16) <= 47)
                    data_bytes = BytesOut.GetDataPosLen(HexDataStr[i], "Zap-Catalog") + data_bytes;
            }
        ...
        ///               5) Receive data acknowledge signal from the hardware - string 'ended'
        ///               6) Close the serial port
        ///               7) Call functions to reload data, and verify the writing
                ...
                if (!serialPort1.IsOpen)
                {
                    ...
                    while (true)
                    {
                        ...
                        if (response.Contains("ack"))
                        {
                            ...
                            /// ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                            /// ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                            /// This block can be used for debugging the data communication between
                            /// software and hardware
                            string get = "";
                            while (true)
                            {
                                get += serialPort1.ReadExisting();
                                /// MessageBox.Show(get);
        ...
        ///              - Compare the most recently read data with the user changed data
        ///                previously placed user changed data in buffers
        ///-----------------------------------------------------------------------------------------------------------
        private void VerifyWriting()
        {
            ...
            for (int i = 0; i < HexDataStr.Length; i++)
            {
                if (HexDataStr[i].Length > 1)
                {
                    ...
                    if (error)
                    {
                        ErrorStr += "ID Tag " + (i + 1).ToString() + " -> " + BytesOut.GetDataFieldNames(strbuf);
                        ErrorStr += "\r\n";
                    }
                }
            }
        ...
        ///              i) Organize the sequence of available data to write - Barcode, DSR, and Catalog number
        ///              ii) Convert ASCII to hex data and add ASCII data length and CRC
        ///                  => data JHNJK will be -> 5 + JHJK + {crc of (5 + JHJK)}
        ///-----------------------------------------------------------------------------------------------------------
        private string ComputeDataToWrite(string barcode, string dsr, string catalog, string currentBarcode,
                                                             string currentDsr, string currentCatalog)
        {
            ...
            if (barcode.Length > 0)
            {
                if (currentDsr.Length > 0)
                    barcode += BytesOut.CharToHex(currentDsr, ref array) + crc.crc8(array);
                if (currentCatalog.Length > 0)
                    barcode += BytesOut.CharToHex(currentCatalog, ref array) + crc.crc8(array);
                return barcode;
            }
            else
            {
                ...
                {
                    if (currentCatalog.Length > 0)
                        dsr += BytesOut.CharToHex(currentCatalog, ref array) + crc.crc8(array);
                    return dsr;
                }
            }
            return catalog;
        ...
        ///                Optional parameter -> progressValue
        /// Description: 1) Request for data bytes from the ID Tags, send read signal
        ///              2) Receive data bytes from the ID Tags as character values and store
        ///                 them as a string. Update the progress bar.
        ///              3) Receive end signal of data bytes from the Hardware, and
        ///                 terminate function.
        ///-----------------------------------------------------------------------------------------------------------
        private void DataRequest(SerialPort serialPort1, int progresValue = 0)
        {
            serialPort1 = new SerialPort(PORT);
            string data = "";
            serialPortInit(serialPort1);
            if (!serialPort1.IsOpen)
            {
                ...
                while (true)
                {
                    ...
                    {
                        data = data + "  " + response;
                        if (progresValue < data.Length && progresValue != 0)
                        {
                            this.progressBar1.Value = progresValue;
                            progresValue += 2;
                        }
                        else if (progresValue == 0)
                        {
                            progresValue += data.Length;
                            this.progressBar1.Value = progresValue; 
                        }
                        else
                        {
                        ...
                        {
                            BytesIn.SerialStrToBytes(data, ref DataBuffer);
                            /// MessageBox.Show(data);
                            serialPort1.Close();
                            return;
                        }
        ...
        /// Description: Refresh or reload button - Reload data from the ID Tags
        ///-----------------------------------------------------------------------------------------------------------
        ...
        /// Description: Reload data from the ID Tags
        ///-----------------------------------------------------------------------------------------------------------
        private void ReloadData(int progressValue)
        {
            this.progressBar1.Value = progressValue;
            /// Checkboxs init
            EnableRecordDataTagList();
            EnableHexDataTagList();
            DataBuffer = new string[NUMBER_OF_TAGS];
            for (int i = 0; i < NUMBER_OF_TAGS; i++)
                DataBuffer[i] = " ";
            /// Request data 
            DataRequest(serialPort1, progressValue);
            this.progressBar1.Value = ProgressMax;
            ClearAllDataFields();
        ...
        {
            BytesIn.GetRecordList(HexDataStr[0], ref DataRecord);
            DisplayDataRecord(DataRecord);
        ...
        {
            BytesIn.GetRecordList(HexDataStr[1], ref DataRecord);
            DisplayDataRecord(DataRecord);
            /// MessageBox.Show(DataRecord[0]);
        }
        ...
        {
            BytesIn.GetRecordList(HexDataStr[2], ref DataRecord);
            DisplayDataRecord(DataRecord);
        }
        ...
        {
            BytesIn.GetRecordList(HexDataStr[3], ref DataRecord);
            DisplayDataRecord(DataRecord);
        }
        ...
        {
            BytesIn.GetRecordList(HexDataStr[4], ref DataRecord);
            DisplayDataRecord(DataRecord);
        }
        ...
        {
            BytesIn.GetRecordList(HexDataStr[5], ref DataRecord);
            DisplayDataRecord(DataRecord);
        }
